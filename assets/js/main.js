...
/* Rodada final institucional: aviso de cookies */
function setupCookieConsent(){
  try{
    const banner=document.querySelector('[data-cookie-banner]');
    if(!banner)return;
    const key='oe_cookie_consent_v1';
    if(localStorage.getItem(key)==='accepted'){banner.setAttribute('hidden','');return;}
    banner.removeAttribute('hidden');
    const btn=banner.querySelector('[data-cookie-accept]');
    if(btn)btn.addEventListener('click',()=>{localStorage.setItem(key,'accepted');banner.setAttribute('hidden','');if(typeof trackEvent==='function')trackEvent('cookie_consent_accept',{event_category:'privacy',page_path:location.pathname});});
  }catch(error){try{console.warn(error)}catch(e){}}
}
